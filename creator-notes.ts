 ...
 *
 * 来源: opencli/src/clis/xiaohongshu/creator-notes.ts
 */
...
async function fetchCreatorNotesByApi(page: any, limit: number): Promise<CreatorNoteRow[]> {
  ...
  for (let pageNum = 1; pageNum <= maxPages && notes.length < limit; pageNum++) {
    ...
    if (!items.length) {
      ...
      const data = intercepted.find((entry: any) => Array.isArray(entry?.data?.note_infos)) as CreatorAnalyzeApiResponse | undefined;
